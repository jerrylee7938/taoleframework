//全局变量resultData图片上传成功后保存接口返回数据
var resultData = new Array();
//全局变量url保存各个业务传递过来的要上传的后台接口地址
var url = "";
...
var photoViewer = Ext.create('Ext.Component', {
	...
	html : '<div class="thumb-viewer-title">'
			...
			+ '<div class="thumb-viewer-close-item"></div>' + '</div>'
			+ '</div>' + '<div class="thumb-viewer-wrap">' + '<img />'
			+ '</div>' + '<div class="thumb-viewer-info">' + '</div>'
			+ '<div class="thumb-viewer-toolbar"></div>',
	listeners : {
		show : function() {
			photoViewer.navPrevBtn.setDisabled(photoViewer.viewIndex == 0);
			photoViewer.navNextBtn.setDisabled(photoViewer.viewIndex == store
					.getCount()
					- 1);
			Ext.getBody().mask();
		},
		...
		resize : function() {
			...
			positionImage(store.getAt(photoViewer.viewIndex), null, null,
					false, true);
		...
		afterrender : function(c) {
			...
			photoViewer.zoomInBtn = Ext.create('Ext.Component', {
						...
						listeners : {
							render : function() {
								this.el
										.addClsOnClick('toolbar-item-zoom-in-click');
			...
			photoViewer.zoomOutBtn = Ext.create('Ext.Component', {
						...
						listeners : {
							render : function() {
								this.el
										.addClsOnClick('toolbar-item-zoom-out-click');
					...
					render : function() {
						...
						this.el.on('click', function() {
									...
									if (photoViewer.rotate) {
										photoViewer.rotate = photoViewer.rotate > 0
									...
									photoViewer.el.down('img').setStyle({
										'-webkit-transform' : 'rotate('
												+ photoViewer.rotate + 'deg)',
										'ease-in-out' : '0.2s'
									});
									positionImage(
											store.getAt(photoViewer.viewIndex),
			...
			photoViewer.rotateRighttBtn = Ext.create('Ext.Component', {
				...
				listeners : {
					render : function() {
						this.el
								.addClsOnClick('toolbar-item-rotate-right-click');
						this.el.on('click', function() {
									photoViewer.rotated = true;
									if (photoViewer.rotate) {
										photoViewer.rotate = photoViewer.rotate > 0
			...
			Ext.create('Ext.tip.ToolTip', {
						...
						listeners : {
							beforeshow : function(tip) {
								...
								tip.update(Ext.String.format(tiptpl, record
												.get('name'), record
												.get('type'), record
												.get('size'), record
												.get('trueWidth')
												+ "×"
												+ record.get('trueHeight'),
										Ext.Date.format(record.get('lastmod'),
												'Y年m月d日 H时i分')));
 ...
 * @param {} callback  回调
 * @param {} allowMultiUpload 是否允许上传多张图片
 */
function uploadPicture(requestUrl, callback, scope, allowMultiUpload){
	url = requestUrl;
	// 创建相册窗口
	var win = Ext.create('Ext.window.Window', {
		...
		scope: scope,
		modal: true,
		afterChooseFn: callback||Ext.emptyFn,
		buttons : [
			{
				text : '上传', //上传按钮
				iconCls : 'upload',
			...
			},
			{
				...
				iconCls : 'upload',
				handler : function(){
					if(win.afterChooseFn.call(win.scope, resultData)!==false){//回调函数没有返回false
						resultData = [];
						win.close();
						deleteAll();
					}
		...
		listeners : {
			afterrender : function(){
				addBtn.el.createChild({
		        	...
		        	if(allowMultiUpload){
		        		if(store.getCount() == 1){
		        			Ext.Msg.alert("提示", "对不起，本次操作只能上传一张图片！");
		        			return;
		        		}
		        	}
...
//							Ext.get(view.getNode(g))
//									.down('div.thumb-loading-tip').hide();
						}
					}(p));
...
function viewPhoto(photo) {
	if (photo) {
		...
		info.update('<h1>文件信息:</h1><p>'
				+ Ext.String.format(tiptpl, photo.get('name'), photo
								.get('type'), photo.get('size'), photo
								.get('trueWidth')
								+ "×" + photo.get('trueHeight'), Ext.Date
								.format(photo.get('lastmod'), 'Y年m月d日 H时i分'))
				+ '</p>');
	}
}
...
function isRotate() {
	...
	return ((rotate == 90 || rotate == 270 || rotate == -90 || rotate == -270) && photoViewer.rotated === true);
}

function positionImage(photo, width, height, fit, notClearRotate) {
	width = width
			|| (!isRotate() ? photo.data.trueWidth : photo.data.trueHeight);
	height = height
			|| (!isRotate() ? photo.data.trueHeight : photo.data.trueWidth);
