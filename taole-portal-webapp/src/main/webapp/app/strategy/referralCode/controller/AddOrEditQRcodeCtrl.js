Ext.define('Taole.strategy.referralCode.controller.AddOrEditQRcodeCtrl', {
    extend: 'Ext.app.Controller',
    views: [
        "Taole.strategy.referralCode.view.AddOrEditQRcodeWindow",
        "Taole.strategy.referralCode.view.QRcodeImageUploadWindow"
    ],
    refs: [
        {
            ref: 'window',
            selector: 'addOrEditQRcodeWindow'
        },
        {
            ref: 'window',
            selector: 'qrcodeImageUploadWindow'
        },
        {
            ref: 'form',
            selector: 'addOrEditQRcodeWindow>form'
        },
        {
            ref: 'grid',
    ...
    init: function () {
        if (Taole.strategy.referralCode.controller.AddOrEditQRcodeCtrl.isInit)return;

        Taole.strategy.referralCode.controller.AddOrEditQRcodeCtrl.isInit = true;
        this.control({
            'addOrEditQRcodeWindow': {
            ...
            'addOrEditQRcodeWindow button[action=add]': {//添加图片
                click: function () {
            ...
            'addOrEditQRcodeWindow button[action=save]': {//保存
                click: function (btn) {
            ...
            'addOrEditQRcodeWindow button[action=cancel]': {//取消
                click: function () {
                    this.getWindow().close();
                }
            },
            'qrcodeImageUploadWindow button[action=confirm]': {//确定
                click: function (btn) {
            ...
            'qrcodeImageUploadWindow button[action=cancel]': {//取消
                click: function (btn) {
                    btn.up("window").close();
                }
            }
    ...
    addAppAdFieldSet: function(data){
    	var bannerForm = Ext.ComponentQuery.query("addOrEditQRcodeWindow>form")[0];
...
//                    },
    ...
    get: function(posId, successFn, failureFn, scope){
    	Ext.Ajax.request({
			url: APP_ADCONTENT_QUERY + "?posId=111",
			success: function(response){
				var data = Ext.decode(response.responseText);
				if(getResp(data)){
					successFn.call(this, data);
