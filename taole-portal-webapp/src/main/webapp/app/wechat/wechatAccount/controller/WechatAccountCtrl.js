Ext.define('Taole.wechat.wechatAccount.controller.WechatAccountCtrl', {
    extend: 'Ext.app.Controller',
    views: [
    	'Taole.wechat.wechatAccount.view.WechatAccountPanel'
    ],
    refs: [
    {
    	 ref: 'form',
         selector: 'wechatAccountPanel>form'
    },
    {
    	 ref: 'grid',
         selector: 'wechatAccountPanel>grid'
    }
    ],
    init: function() {
    	this.control({
    		'wechatAccountPanel>form button[action=query]': {//查询
    			click: function(){			    	
					this.getGrid().store.load();
    			}
    		},
    		'wechatAccountPanel>form button[action=reset]': {//重置
    			click: function(){			    	
					this.getForm().form.reset();
    			}
    		},
    		'wechatAccountPanel>grid': {
    		...
    		'wechatAccountPanel>grid button[action=add]': {//新增
    			click: function(){
    				this.edit();
    			}
    		},
    		'wechatAccountPanel>grid button[action=update]': {//修改
    			click: function(){
    		...
    		'wechatAccountPanel>grid button[action=remove]': {//删除
    			click: function(){
    				var selRows = this.getGrid().getSelectionModel().getSelection();
    				if(selRows.length == 0){
    		...
    		'wechatAccountPanel>grid button[action=configure]': {//消息配置
    			click: function(){
    				console.log('11');
    				var selRows = this.getGrid().getSelectionModel().getSelection();
    				if(selRows.length == 0){
    				...
    				Ext.create("Taole.wechat.wechatNews.controller.WechatNewsConfigureCtrl").init();
			    	Ext.create("widget.wechatNewsConfigureWindow",{
			    		scope: this,
    		...
    		'wechatAccountPanel>grid button[action=retrieve]': {//详情
    			click: function(){
    				var selRows = this.getGrid().getSelectionModel().getSelection();
    				if(selRows.length == 0){
    					Ext.Msg.alert("提示", "请选择记录");
    ...
    edit: function(appAdPositionId, isView, isRetrieve){
		Ext.create("Taole.wechat.wechatAccount.controller.AddOrEditPositionCtrl").init();
