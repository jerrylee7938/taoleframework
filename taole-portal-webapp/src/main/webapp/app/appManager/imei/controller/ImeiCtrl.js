Ext.define('Taole.appManager.imei.controller.ImeiCtrl', {
    extend: 'Ext.app.Controller',
    views: [
    	'Taole.appManager.imei.view.ImeiPanel'
    ],
    refs: [
    {
    	 ref: 'form',
         selector: 'imeiPanel>form'
    },
    {
    	 ref: 'grid',
         selector: 'imeiPanel>grid'
    }
    ],
    init: function() {
    	this.control({
    		'imeiPanel>form button[action=query]': {//查询
    			click: function(){			    	
					this.getGrid().store.load();
    			}
    		},
    		'imeiPanel>form button[action=reset]': {//重置
    			click: function(){			    	
    		...
    		'imeiPanel>grid button[action=add]': {//新增
    			click: function(){
    				var arr = new Array();
    				arr.push({
    					"imei": ""
    		...
    		'imeiPanel>grid button[action=remove]': {//删除
    			click: function(){
    				...
    				Ext.Msg.confirm("提示", "确定删除选中的IMEI吗？", function(bt){
						if(bt=='yes'){
    ...
    del: function(imeiIds, successFn, failureFn){
    	Ext.Ajax.request({
			...
			success: function(response){
				var data = Ext.decode(response.responseText);
				if(getResp(data)){
					Ext.Msg.alert("提示", "IMEI删除成功！", successFn, this);
				}else{
					Ext.Msg.alert('提示','IMEI删除失败!<br/>'+data.result_code+":"+data.result_desc, failureFn, this);
				}
			},
			failure: function(){
				Ext.Msg.alert("提示", "IMEI删除失败", failureFn, this);
			},
			scope: this
		});
    }
