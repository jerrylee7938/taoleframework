Ext.define('Taole.appManager.appAd.controller.AddOrEditPositionCtrl', {
    extend: 'Ext.app.Controller',
    views: [
    	"Taole.appManager.appAd.view.AddOrEditPositionWindow"
    ],
    refs: [
    {
    	 ref: 'window',
    ...
    init: function() {  
    	if(Taole.appManager.appAd.controller.AddOrEditPositionCtrl.isInit)return;

    	Taole.appManager.appAd.controller.AddOrEditPositionCtrl.isInit = true;
    	...
    	Ext.Ajax.request({
			...
			success: function(response){
				...
				if(getResp(data)){
					...
					Ext.Msg.alert('提示','获取APP广告位置详情失败!<br/>'+data.result_code+":"+data.result_desc, failureFn, this);
				}
			},
			failure: function(){
				Ext.Msg.alert("提示", "获取APP广告位置详情失败，请联系管理员！", failureFn, this);
			},
			scope: scope
		});
    },
		...
		Ext.Ajax.request({
			...
			success: function(response){
				...
				if(getResp(data)){
					...
					Ext.Msg.alert("提示", "广告位置保存失败", failureFn, this);
				}
			},
			failure: function(){
				Ext.Msg.alert("提示", "广告位置保存失败，请联系管理员", failureFn, this);
			},
			scope: scope||this
		});
    }
